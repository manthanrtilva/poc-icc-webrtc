      ...
      if (line == "exit") {
        ...
        if (task_queue_) {
          task_queue_->PostTask(
              [this] { callback_->DisconnectFromServer(); });
        }
        break;
        ...
        if (pid != -1) {
          if (task_queue_) {
            task_queue_->PostTask(
                [this, pid] { callback_->ConnectToPeer(pid); });
          }
        } else {
          RTC_LOG(LS_WARNING) << "Usage: pc <pid>";
        }
      } else if (line == "sd") {
        if (task_queue_) {
          task_queue_->PostTask(
        ...
        if (task_queue_) {
          task_queue_->PostTask(
              [this] { callback_->DisconnectFromCurrentPeer(); });
        }
      } else if (line == "l") {
        if (task_queue_) {
          task_queue_->PostTask(
              [this] { for(auto& a :callback_->GetPeers()){

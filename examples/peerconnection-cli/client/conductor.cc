...
#include "peerconnection-cli/client/conductor.h"
...
#include "peerconnection-cli/client/defaults.h"
#include "peerconnection-cli/client/main_wnd.h"
#include "peerconnection-cli/client/peer_connection_client.h"
...
bool Conductor::CreatePeerConnection() {
  ...
  if (error_or_peer_connection.ok()) {
    ...
    init.ordered = true;
    auto error_or_data_channel =
        peer_connection_->CreateDataChannelOrError("data_channel", &init);
    RTC_LOG(LS_ERROR);
    if (error_or_data_channel.ok()) {
      RTC_LOG(LS_ERROR);
      data_channel_ = std::move(error_or_data_channel.value());
      data_channel_->RegisterObserver(this);
      data_channel_->Send(webrtc::DataBuffer("hello from caller"));
    }

  } else {
    RTC_LOG(LS_ERROR) << "Failed to create PeerConnection: "
                      << ToString(error_or_peer_connection.error().type())
...
void Conductor::OnStateChange() {
  RTC_LOG(LS_INFO) << __FUNCTION__ << " " << data_channel_->state();
...
void Conductor::OnMessage(const webrtc::DataBuffer& buffer) {
  ...
  RTC_LOG(LS_INFO) << __FUNCTION__ << " " << message;
}
...
void Conductor::OnDataChannel(
  ...
  RTC_LOG(LS_INFO) << __FUNCTION__ << " " << channel->label();
...
// void Conductor::PrintPeers() {

// }
void Conductor::PrintPeers() {
  for (auto& a : client_->peers()) {
    RTC_LOG(LS_INFO) << "Peer id: " << a.first << " Peer name: " << a.second;
  }
}
void Conductor::SendDataMessage(const std::string& message) {
  RTC_LOG(LS_INFO);
  if (data_channel_) {
    RTC_LOG(LS_INFO);
